...
public class SysRoleUserService {
	...
	@ArchivesLog(actionName = "【操作类：SysRoleService】", option = "【方法名：selectListByUserId，作用：通过用户ID获取用户角色信息】")
	 ...
	 * if(StringUtils.isBlank(roleId)) { return new ArrayList<SysRoleMenu>(); }
	 * SysRoleMenu po = new SysRoleMenu(); po.setRoleId(roleId);;
	 * List<SysRoleMenu> list = this.sysRoleMenuMapper.selectList(po); return
	 * list; }
	 * 
	 *//**
		 * 新增，根据角色id，权限字符串
		 ...
		 * this.deleteByRoleId(roleId); String[] ss = s.split(",", -1);
		 * List<SysRoleMenu> list = new ArrayList<SysRoleMenu>(); for (String
		 * string : ss) { if(StringUtils.isBlank(string)) { continue; } String[]
		 * strings = string.split("_", -1); String menuId = strings[0]; String
		 * operations = strings[1]; String uuid = UUIDUtil.createUUID();
		 * SysRoleMenu po = new SysRoleMenu(); po.setRmId(uuid);
		 * po.setRoleId(roleId); po.setMenuId(menuId);
		 * po.setOperations(operations); list.add(po);
		 * this.sysRoleMenuMapper.insert(po); } return list; }
		 * 
		 * public Boolean deleteByRoleId(String roleId) { Boolean bl =
		 * this.sysRoleMenuMapper.deleteByRoleId(roleId); return bl; }
		 */
}
