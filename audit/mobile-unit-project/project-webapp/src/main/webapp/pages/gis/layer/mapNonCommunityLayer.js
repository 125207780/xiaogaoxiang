...
(function() {
	 ...
	 */
	var NonCommunityLayer = BMapLib.NonCommunityLayer = function(setRange, uId, poiInfo, orgId) {
		this.conf = {};
		this.overlayList = [];
	...
	NonCommunityLayer.prototype = new BMap.Overlay();

	NonCommunityLayer.prototype.initialize = function(map) {
    	this._map = map;
    	var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("nonCommunity" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllNonCommunityByOrgId", 
			...
			success: function(list) {
				...
				// bd 1 end
				var img = new Image();
				img.src = $.cxt + "/pages/gis/layer/nonCommunity.png";
...
//							me.overlayList.push(marker);
				        	...
				        	click: function(item) {
				            	...
				            	if(item && me._map.getZoom() > 10) {
				                	...
				                	topwindow.showWindow({
										...
										title: '未入格小区信息',
										data: {uid: item.uid},
										url: $.cxt + "/pages/gis/layer/nonCommunity.jsp",
										bottons: [{
											title: "关闭" ,
											fun: function() {
												topwindow.removeWindow();
	...
	NonCommunityLayer.prototype.draw = function() {
    	this.drawOverlay();
    }
    
	NonCommunityLayer.prototype.drawOverlay = function() {
		if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.NonCommunityLayer = NonCommunityLayer;
})()
