...
(function() {
     ...
     */
	var OfficeBuildingLayer = BMapLib.OfficeBuildingLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	OfficeBuildingLayer.prototype = new BMap.Overlay();

	OfficeBuildingLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("officeBuilding" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllOfficeBuildingOrgId", 
					        	...
					        	click: function(item) {
					            	if(item && me._map.getZoom() > 10) {
					                	topwindow.showWindow({
											...
											title: '写字楼基础信息',
											data: {uid: item.uid},
											url: $.cxt + "/pages/gis/layer/officeBuilding.jsp",
											bottons: [{
												title: "关闭" ,
												fun: function() {
	...
	OfficeBuildingLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	OfficeBuildingLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.OfficeBuildingLayer = OfficeBuildingLayer; 
})()
