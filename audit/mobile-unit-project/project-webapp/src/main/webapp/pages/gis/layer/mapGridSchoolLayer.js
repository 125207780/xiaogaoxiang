...
(function() {
	/** 
	 * @exports NonCdGroupLayer as BMapLib.NonCdGroupLayer
     */
	var GridSchoolLayer = BMapLib.GridSchoolLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	GridSchoolLayer.prototype = new BMap.Overlay();

	GridSchoolLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("gridSchool" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectGridSchoolOrgId", 
					        	...
					        	click: function(item) {
					            	if(item && me._map.getZoom() > 10) {
					                	topwindow.showWindow({
											...
											title: '网格学校信息',
											data: {uid: item.uid},
											url: $.cxt + "/pages/gis/layer/gridSchoolDetail.jsp",
											bottons: [{
												title: "关闭" ,
												fun: function() {
	...
	GridSchoolLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	GridSchoolLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.GridSchoolLayer = GridSchoolLayer; 
})()
