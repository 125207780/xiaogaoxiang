$(function() {
	$('#grid-table').jqGrid({
		url : $.cxt + "/testdhysysuser/testdhyselectpagelist",//获取数据地址
	...
	//查询按钮
	$("#testDhySearchUserList").click(function(){
		reloadJqGrid("search");
	});
	//新建按钮
	$("#testDhyCreateUserBtn").click(function(){
		testDhyShowSysUserForm();
	})
})
//格式化最后一列
var renderOperation = function(cellvalue, options, cell) {
	var html = ""; 
	html += "<a onclick=\"testDhyShowSysUserForm('" + cell.userId + "')\" href=\"#\">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;";
	html += "<a  onclick=\"testDhyDeleteSysUser('" + cell.userId + "')\" href=\"#\">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;"
	html += "<a  onclick=\"testDhyShowSysUserRoleForm('" + cell.userId + "')\" href=\"#\">分配角色</a>"
	return html;
}
//重新加载表格
...
//角色分配弹出窗口
var testDhyShowSysUserRoleForm = function(userId) {
...
//弹出窗口
var testDhyShowSysUserForm = function(id) {
	topwindow.showWindow({
		//窗口名称
		title : "用户编辑测试",
		//参数
		data : {id : id},
		//url
		url : $.cxt + "/testdhysysuser/testdhyshowsysuserform",	
		bottons : [{
			title : "确认测试",
			fun : function() {
				//校验，参数为表单父级id
				if(topcheckoutForm("testDhySysUserForm")){
					$.ajax({
						url : $.cxt + "/testsysuser/testinsertorupdatesysuser", 
						data : topgetObjByObj($("#testDhySysUserForm .formField")),
						type: "POST",
						success : function(data) {
							//重新加载表格
							reloadJqGrid();
...
var testDhyDeleteSysUser = function(id) {
