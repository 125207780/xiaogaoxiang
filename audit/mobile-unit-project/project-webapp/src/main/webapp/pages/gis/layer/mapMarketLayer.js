...
(function() {
     ...
     */
	var MarketLayer = BMapLib.MarketLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	MarketLayer.prototype = new BMap.Overlay();

	MarketLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("market" == allOverlay[i].poiInfo) {
				        	...
				        	click: function(item) {
				            	if(item && me._map.getZoom() > 15) {
				                	topwindow.showWindow({
										...
										title: '聚类市场信息',
										data: {uid: item.uid},
										url: $.cxt + "/pages/gis/layer/marketInfo.jsp",
										bottons: [{
											title: "关闭" ,
											fun: function() {
	...
	MarketLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	MarketLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.MarketLayer = MarketLayer; 
})()
