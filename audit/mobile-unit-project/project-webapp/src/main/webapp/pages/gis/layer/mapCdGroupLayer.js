...
(function() {
	/** 
	 * @exports CdGroupLayer as BMapLib.CdGroupLayer
     */
	var CdGroupLayer = BMapLib.CdGroupLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	CdGroupLayer.prototype = new BMap.Overlay();

	CdGroupLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("allCDGroup" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllCdGroupByOrgId", 
	...
	CdGroupLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	CdGroupLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.CdGroupLayer = CdGroupLayer; 
})()
