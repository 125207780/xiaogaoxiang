	...
	var lastMonth = dailyInitDate1();
	   ...
	   grid.jqGrid({ 
		   ...
		   colModel: [ 
			...
			rowList : [ 10,20, 30 ],
			viewrecords: false, 
			pager : '#dailyReportForm1InfoListPagers',
			gridComplete : dailyReportForm1setNiceScroll,
	   ...
	   //查询按钮
	   $("#dailySearchList1").click(function(){
		   dailyReloadBasicUnitJqGrid1("search");
		 });
	   //导出
		$("#getReportForm1Export").click(function(){
			var statisMonth = encodeURI(encodeURI($("#AccountPeriod1").val()));
			window.location.href=$.cxt +"/getReportForm/getReportForm1Export?statisMonth="+statisMonth;
		})
}
var dailyReloadBasicUnitJqGrid1 = function(flag) {
	if(flag == undefined) {
		//清空查询条件
		topclear('gridSearch')
	}
	statisMonth = $("#AccountPeriod1").val();
	if(statisMonth == undefined){
		statisMonth = "";
	}
	
	$("#dailyReportForm1InfoListGrid").jqGrid('setGridParam',{    
        postData : {
...
function dailyInitDate1(){
	...
	$("#AccountPeriod1").val(lastMonth);
	return lastMonth;
}
function dailyReportForm1setNiceScroll(){
