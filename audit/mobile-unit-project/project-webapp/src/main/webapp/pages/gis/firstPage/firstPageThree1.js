$(function() {
	...
	// 当前登录人层级是省和地市层级的，则首页显示地图和echart图
		...
		// 获取首页账期文本框
		  orgId = $("#hiddenOrgId").val() == null || $("#hiddenOrgId").val() == '' ? nowOrgId : $("#hiddenOrgId").val();
		$.ajax({
...
//	  
//	    //初始化加载所有指标
//	    initPiePercent(orgId,nowOrgLevel);
...
function statisDateHeader() {
	// 环形进度条
	initPiePercent($("#hiddenOrgId").val(),$('#hiddenOrgLevel').val());
}
...
function initPiePercent(orgId,orgLevel) {
	...
	$.ajax({
		url: $.cxt + "/firstpagethree/initZBinfo",
		data: {orgId: orgId, orgLevel:orgLevel, statisDate: statisDate},
		type: "POST",
		async : false,
		success: function(result) {
			var json = JSON.parse(result);
			...
			if(json.code == '0') {
				...
				for(var i = 0; i < json.length; i++) { 
					if(i==0){
						indexCode=json[i].INDEX_CODE;
						indexName=json[i].INDEX_NAME;
					}
					htmltv += '<div class="pic" >'
							+ '<div class="picContent" id="'+json[i].INDEX_CODE+'">'
			                + '<div style="height:10px;"> </div>'
			                + '<span class="spanMainTitle">'+json[i].INDEX_NAME+'</span>'
			                + '<div class="zbMainContent">'
			                + '<div class="zbContentLeft">'
			                + '<span>'+json[i].INDEX_DETAIL01_NAME+'</span><br/>'
			                + '<span class="num">'+json[i].INDEX_DETAIL01_VALUE+'</span>'
			                + '</div>'
			                + '<div class="solid">'
			                + '</div>'
			                + '<div class="zbContentRight">'
			                + '<span>'+json[i].INDEX_DETAIL02_NAME+'</span><br/>'
			                + '<span class="num">'+json[i].INDEX_DETAIL02_VALUE+'</span>'
			                + '</div>'
			                + '<div class="showQuestion">'
			                + '<div class="qustion" >'
			                + '<div class="explain" >'+json[i].INDEX_DESC+'</div>'
			                + '<img  class="imgU53" src="'+$.cxt +'/pages/images/u53.png">'
			                + '<img  class="imgU55" src="'+$.cxt +'/pages/images/u55.png">'
			                + '</div>'
			                + '</div>'
			                + '</div>'
			                + '</div>'
				...
				$("#List1").html(htmltv);
				$('.pic').css('width',($('.Cont').width()/4));
				if(json.length!=0){
					initEcharts($("#hiddenOrgId").val(),orgLevel,null,indexCode,indexName,'DAY');
				}else{
	...
	$('.picContent').on("click",function(){
		$('.picContent').css("border","0px");
		$('.picContent').css("backgroundColor","#ffffff");
		this.style.border="1px solid rgba(6, 172, 210, 1)";
		this.style.backgroundColor="rgba(204, 234, 249, 1)";
	});
	 //点击当日加载echart
	$('.zbContentLeft').on("click",function(){
		initEcharts($("#hiddenOrgId").val(),$('#hiddenOrgLevel').val(),this,"","",'DAY');
	});
	//点击当月加载echart
	$('.zbContentRight').on("click",function(){
		initEcharts($("#hiddenOrgId").val(),$('#hiddenOrgLevel').val(),this,"","",'MONTH');
	});
	//点击问号
	$('.imgU53').on("click",function(){
		var showExplain=$(this).parent().find('.explain').text();
		winwarin(showExplain);   	
	});
	$('.imgU55').on("click",function(){
		var showExplain=$(this).parent().find('.explain').text();
		winwarin(showExplain);  
	});
