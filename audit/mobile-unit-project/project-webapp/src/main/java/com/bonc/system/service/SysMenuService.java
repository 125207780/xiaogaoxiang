...
public class SysMenuService {
	...
	public Page<SysMenu> selectPageList(SysMenu po, Integer page, Integer row) {
		...
		Page<SysMenu> pageList = (Page<SysMenu>) this.sysMenuMapper
				.selectList(po);
		return pageList;
	}
	 ...
	 * @return
	 */
	public List<SysMenu> selectList(SysMenu po,
		...
		for (SysMenu sysMenu : list) {// 循环菜单
			...
			List<SysMenuOperation> sysMenuOperationList = this.sysMenuOperationService
					.selectListByMenuId(sysMenu.getMenuId());// 查询菜单对应的操作权限
			if (sysRoleMenu != null) {// 菜单对应的角色权限不为空
				...
				if (!StringUtils.isBlank(sysRoleMenu.getOperations())) {// 如果角色权限有操作权限
					for (SysMenuOperation sysMenuOperation : sysMenuOperationList) {// 循环该菜单对应的操作权限
						if (sysRoleMenu.getOperations().contains(
								sysMenuOperation.getOperationCode() + "#")) {// 判断角色权限中操作权限是否包含对应的操作权限
	...
	public SysMenu insertSysMenu(SysMenu po) {
		...
		this.sysMenuMapper.insertSysMenu(po);
		return po;
	}
	...
	public SysMenu updateSysMenu(SysMenu po) {
		this.sysMenuMapper.updateSysMenu(po);
		return po;
	}
	...
	public List<SysMenu> selectTree(String roleId) {
		List<SysRoleMenu> sysRoleMenuList = this.sysRoleMenuService
				.selectListByRoleId(roleId);// 查询对应的角色权限，用于判断菜单和菜单对应的操作权限checked是否被选中
		SysMenu po = new SysMenu();// 查询对象
		List<SysMenu> list = this.selectList(po, sysRoleMenuList);// 查询所有菜单
		List<SysMenu> paList = new ArrayList<SysMenu>();
		for (SysMenu temp : list) {
			if (temp.getMenuLevel().equals("1")) {
				this.selectTreeChildren(temp, list);// 递归查询子菜单
				paList.add(temp);
			}
		}
		return paList;
	...
	private void selectTreeChildren(SysMenu po, List<SysMenu> list) {
		List<SysMenu> chList = new ArrayList<SysMenu>();
		for (SysMenu temp : list) {
			if (StringUtils.isNotBlank(temp.getParentId())) {
				if (temp.getParentId().equals(po.getMenuId())) {
					this.selectTreeChildren(temp, list);// 继续递归查询子菜单
					chList.add(temp);
				}
			}
		}
		if (chList.size() > 0) {
			po.setChildren(chList);
		}
	}
	...
	private void selectTreeChildren0(SysMenu po, List<SysMenu> list) {

		List<SysMenu> childrenlList = new ArrayList<SysMenu>();
		for (SysMenu temp : list) {
			if (!StringUtils.isBlank(temp.getParentId())) {// 父节点不为空
				if (temp.getParentId().equals(po.getMenuId())) {
					selectTreeChildren0(temp, list);// 继续递归子节点
					childrenlList.add(temp);
				}
			}
		}
		if (childrenlList.size() > 0) {
			po.setChildren(childrenlList);// set子节点
		}
