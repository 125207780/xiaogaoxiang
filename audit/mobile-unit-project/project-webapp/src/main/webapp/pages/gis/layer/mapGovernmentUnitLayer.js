...
(function() {
	/** 
	 * @exports NonAbGroupLayer as BMapLib.NonAbGroupLayer
     */
	var GovernmentUnitLayer = BMapLib.GovernmentUnitLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	GovernmentUnitLayer.prototype = new BMap.Overlay();

	GovernmentUnitLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("governmentUnit" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllGovernmentUnitOrgId", 
					        	...
					        	click: function(item) {
					            	if(item && me._map.getZoom() > 10) {
					                	topwindow.showWindow({
											...
											title: '政府单位基础信息',
											data: {uid: item.uid},
											url: $.cxt + "/pages/gis/layer/governmentUnit.jsp",
											bottons: [{
												title: "关闭" ,
												fun: function() {
	...
	GovernmentUnitLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	GovernmentUnitLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.GovernmentUnitLayer = GovernmentUnitLayer; 
})()
