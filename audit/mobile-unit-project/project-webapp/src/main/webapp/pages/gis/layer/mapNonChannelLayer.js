...
(function() {
	 ...
	 */
	var NonChannelLayer = BMapLib.NonChannelLayer = function(setRange, uId, poiInfo, rangeInfo, orgId) {
		this.conf = {};
		this.overlayList = [];
	...
	NonChannelLayer.prototype = new BMap.Overlay();

	NonChannelLayer.prototype.initialize = function(map) {
    	this._map = map;
    	var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("channel" == allOverlay[i].poiInfo ||"NonChannel" == allOverlay[i].poiInfo ||"allChannel" == allOverlay[i].poiInfo || "ALL_CHANNEL" == allOverlay[i].rangeInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllNonChannelByOrgId", 
			...
			success: function(list) {
				...
				// bd 1 end
				var img = new Image();
				img.src = $.cxt + "/pages/images/gis/alarm1.png";
...
//							me.overlayList.push(marker);
	...
	NonChannelLayer.prototype.draw = function() {
    	this.drawOverlay();
    }
    
	NonChannelLayer.prototype.drawOverlay = function() {
		if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.NonChannelLayer = NonChannelLayer;
})()
