...
(function() {
	/** 
	 * @exports VillageLayer as BMapLib.VillageLayer
     */
	var VillageLayer = BMapLib.VillageLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	VillageLayer.prototype = new BMap.Overlay();

	VillageLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
    	...
    	// 判断是否是选择的基础单元，然后先将整个基础单元的信息给移除，再新增加周边基础单元信息
 	    for(var i = 0; i < allOverlay.length; i++) {
 	    	if("village" == allOverlay[i].poiInfo) {
				        	...
				        	click: function(item) {
				            	if(item && me._map.getZoom() > 15) {
				                	topwindow.showWindow({
										...
										title: '渠道信息',
										data: {uid: item.uid},
										url: $.cxt + "/pages/gis/layer/villageLayer.jsp",
										bottons: [{
											title: "关闭" ,
											fun: function() {
	...
	VillageLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	VillageLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.VillageLayer = VillageLayer; 
})()
