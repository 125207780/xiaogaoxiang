...
(function() {
     ...
     */
	var NonCdGroupLayer = BMapLib.NonCdGroupLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	NonCdGroupLayer.prototype = new BMap.Overlay();

	NonCdGroupLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("nonCDGroup" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllNonCdGroupByOrgId", 
	...
	NonCdGroupLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	NonCdGroupLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.NonCdGroupLayer = NonCdGroupLayer; 
})()
