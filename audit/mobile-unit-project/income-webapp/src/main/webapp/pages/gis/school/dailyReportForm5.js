...
function dailyReportForm5DetailListGrid(){
	var lastMonth = dailyInitDate5();
	   ...
	   grid.jqGrid({ 
		   ...
		   height : 200,
		   width:($("#gview_dailyReportForm5InfoListGrid").width()),
		   autowidth : false,
		   colNames: [ '日期','地市名称','区县名称','学校编码','学校名称','分校区编码','分校区名称','拍照客户数','当前保有客户数','客户保有率'], 
		   colModel: [ 
			...
			gridComplete : dailyReportForm5setNiceScroll,
	 ...
	 //导出
		$("#getReportForm8Export").click(function(){
			var statisMonth = encodeURI(encodeURI($("#dailyAccountPeriod5").val()));
			window.location.href=$.cxt +"/getReportForm/getReportForm8Export?statisMonth="+statisMonth;
		})
}
...
function dailyReportForm5setNiceScroll(){
