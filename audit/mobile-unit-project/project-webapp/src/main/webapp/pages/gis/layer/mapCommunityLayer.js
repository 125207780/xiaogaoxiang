...
(function() {
	/** 
	 * @exports CommunityLayer as BMapLib.CommunityLayer
     */
	var CommunityLayer = BMapLib.CommunityLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	CommunityLayer.prototype = new BMap.Overlay();

	CommunityLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("community" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllCommunityInfoByOrgId", 
	...
	CommunityLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	CommunityLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.CommunityLayer = CommunityLayer; 
})()
