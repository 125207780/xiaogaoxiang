...
(function() { 
    ...
    /**
     * 判断点是否在圆形内（新版本）xiaogaoxiang于2019-05-14增加
     * @param {Point} point 点对象
     * @param {Circle} circle 圆形对象
     * @returns {Boolean} 点在圆形内返回true,否则返回false
     */
    GeoUtils.isPointInCircleNew = function(point, circle){
        //检查类型是否正确
        if (!(point instanceof BMap.Point) || 
        ...
        var dis = GeoUtils.getDistanceNew(point, c);
        if(dis <= r){
            return true;
        } else {
            return false;
     ...
     */
    GeoUtils.getDistanceNew = function(point1, point2){
