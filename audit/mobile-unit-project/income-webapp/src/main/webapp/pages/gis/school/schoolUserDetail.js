...
function schoolUserDetailListGrid(){
	var lastMonth = shcoolUserinitDate();
 ...
 //查询按钮
	...
	$("#export").click(function(){
		statisMonth = $("#shcoolUserAccountPeriod1").val();
		...
		$("#APPFiling option:selected").each(function(){
			if(appFiling =="请选择..."){
				
			}else{
				appFiling.push($(this).val()); //这里得到的就是
			}
	    });
		...
		window.location.href=$.cxt +"/schoolUserDeatil/export?statisMonth="+statisMonth+"&actviTypeId="+ contractType+"&usrType="+ stockAdditions+"&feeLevelId="+ incomeFiling+"&gprsLevelId="+ flowFiling+"&schoolId="+schoolId+"&voiceLevelId="+ phoneticFiling+"&appTypeId="+ encodeURI(encodeURI(JSON.stringify(appFiling)))+"&discFeeId="+ taocanFiling;
	})
//	
...
var reloadBasicUnitJqGrid = function(flag) {
	...
	$("#schoolUserDetailListGrid").jqGrid('setGridParam',{ 
        postData : {
        	...
        	appTypeId:JSON.stringify(appFiling),
...
function shcoolUserinitDate(){
	var lastMonth = "";
	$.ajax({
		url : $.cxt + "/schoolMap/findByDate", 
		type: "POST",
		data:{
		}, 
		async : false,
		success : function(data) {
			...
			if(json.code == '0'){
				lastMonth = json.msg
				$("#shcoolUserAccountPeriod1").datepicker({
			 		language : "zh-CN",
					todayHighlight : true,
					format : 'yyyymm',
					autoclose : true,
				...
				$("#shcoolUserAccountPeriod1").val(lastMonth);
			}
		}
	})
	return lastMonth;
