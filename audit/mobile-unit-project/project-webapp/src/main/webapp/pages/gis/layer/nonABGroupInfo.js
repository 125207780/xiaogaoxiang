$(function(){
	getChannelInfo();//渠道基础信息
	getChannelInfoDay();//渠道日指标信息
	initDate();//初始化日期组件
	OverContent();
	abnormal_info();
	cost_whole();
...
function getChannelInfo(){
	...
	$("#chnlStar").empty();
	$("#chnlName").empty();
	$("#chnlCapacityScore").empty();
	$("#chnlAddr").empty();
	$("#operName").empty();
	$("#chnlTypeLevel1").empty();
	$("#chnlMngrName").empty();
	$("#chnlTypeLevel2").empty();
	$("#chnlMngrNumber").empty();
	$("#openDate").empty();
	$("#useArea").empty();
	$("#chnlCounterNum").empty();
	$("#chnlClerkNum").empty();
	$.ajax({
		...
		success : function(json){
			...
			if(data.code == '0'){
				...
				$("#chnlMngrName").append(data.data[0].CHNL_MNGR_NAME == null || data.data[0].CHNL_MNGR_NAME == "" ? 0 : data.data[0].CHNL_MNGR_NAME);
				$("#chnlTypeLevel2").append(data.data[0].CHNL_TYPE_LEVEL2);
				$("#chnlMngrNumber").append(data.data[0].CHNL_MNGR_NUMBER == null || data.data[0].CHNL_MNGR_NUMBER == "" ? 0 : data.data[0].CHNL_MNGR_NUMBER);
				$("#openDate").append(data.data[0].OPEN_DATE);
				$("#useArea").append(data.data[0].USE_AREA == null || data.data[0].USE_AREA == "" ? 0 : data.data[0].USE_AREA);
				$("#chnlCounterNum").append(data.data[0].CHNL_COUNTER_NUM);
				$("#chnlClerkNum").append(data.data[0].CHNL_CLERK_NUM == null || data.data[0].CHNL_CLERK_NUM == "" ? 0 : data.data[0].CHNL_CLERK_NUM);
			}
		}
	})
}

function getChannelInfoDay() {
	var channelId = $("#channelId").text();
	$.ajax({
