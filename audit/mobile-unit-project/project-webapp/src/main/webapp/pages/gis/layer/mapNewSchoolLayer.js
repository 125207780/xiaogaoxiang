...
(function() {
	/** 
	 * @exports NewSchoolLayer as BMapLib.NewSchoolLayer
     */
	var NewSchoolLayer = BMapLib.NewSchoolLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	NewSchoolLayer.prototype = new BMap.Overlay();

	NewSchoolLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
    	...
    	// 判断是否是选择的基础单元，然后先将整个基础单元的信息给移除，再新增加周边基础单元信息
 	    for(var i = 0; i < allOverlay.length; i++) {
 	    	if("school" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllNewSchoolByOrgId", 
			data: {setRange: this.setRange, uId: this.uId, poiInfo: this.poiInfo, orgId: this.orgId},
			type: "POST",
			loading: false,
			success: function(list) {
				var img = new Image();
				img.src = $.cxt + "/pages/gis/layer/school.png";
				        	...
				        	click: function(item) {
				            	if(item && me._map.getZoom() > 15) {
				                	topwindow.showWindow({
										...
										title: '聚类市场信息',
										data: {uid: item.uid},
										url: $.cxt + "/pages/gis/layer/newSchoolInfo.jsp",
										bottons: [{
											title: "关闭" ,
											fun: function() {
	...
	NewSchoolLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	NewSchoolLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.NewSchoolLayer = NewSchoolLayer; 
})()
