...
(function() {
    ...
    ChannelLayer.prototype.initialize = function(map) {
		...
		var radioLength = $("<input placeholder='范围设置' type='number' min='0' max='1000' id='setRange' style='width:100px' oninput='value=value.replace(/[^\\d]/g,\"\")'>");//只允许输入数字(整数：小数点不能输入)
        ...
        $.ajax({
			url : $.cxt + "/gridCommon/selectAllChannelInter", 
				                	 ...
				                	 if(item&&me._map.getZoom()<18 && me._map.getZoom()>15){
				                		 ...
				                		 if(me.nowItem) {
				                			 if(me.nowItem == item){ //当前选择图标未改变
				                				 ...
				                				 me.nowItem = item;
				                				 if(setRange == ""){
					                				 item.circle.setRadius(1000);
					                				
					                			 }else{
					                				 item.circle.setRadius(setRange)
