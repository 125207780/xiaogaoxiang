...
public class BlSysUserService implements SysUserServiceI {

	@Resource
	private BlSysUserMapper testSysUserMapper;

	public SysUser selectListByLogin(String loginId) {
		SysUser po = new SysUser();
		po.setLoginId(loginId);
		List<SysUser> list = this.testSysUserMapper.selectList(po);
		if (list.size() == 1) {
			return list.get(0);
		}
		return null;
	...
	public Page<SysUser> selectPageList(SysUser sysUser, Integer page, Integer row) {
		...
		Page<SysUser> pageList = (Page<SysUser>) this.testSysUserMapper.selectList(sysUser);
		return pageList;
	}

	public SysUser selectSysUserById(String id) {
		SysUser sysUser = this.testSysUserMapper.selectSysUserById(id);
		return sysUser;
	}
	...
	public SysUser insertSysUser(SysUser sysUser) throws NoSuchAlgorithmException {
		...
		this.testSysUserMapper.insertSysUser(sysUser);
		return sysUser;
	}

	public SysUser updateSysUser(SysUser sysUser) {
		this.testSysUserMapper.updateSysUser(sysUser);
		return sysUser;
	}

	public Boolean deleteSysUserById(String id) {
		Boolean bl = this.testSysUserMapper.deleteSysUserById(id);
		return bl;
	}

	public Boolean selectCheck(SysUser sysUser) {
		Integer i = this.testSysUserMapper.selectCheck(sysUser);
		if (i > 0) {
			return false;
		}
		return true;
