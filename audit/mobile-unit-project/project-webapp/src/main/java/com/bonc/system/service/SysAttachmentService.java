	...
	public void fileDown(String attchId, HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			...
			if (list != null && list.size() > 0) {
				...
				// FtpClientUtil.fileDown(entity.getFilePath(),
				// entity.getRealName(), request, response);
				FtpClientUtil.ftpDownFile(server_id, Integer.parseInt(server_port), server_user, server_pwd, entity.getFilePath(), entity.getRealName(),
						request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
