...
(function() {
     ...
     */
	var NonAbGroupLayer = BMapLib.NonAbGroupLayer = function(setRange, uId, poiInfo, orgId) {
    	this.conf = {};
    	this.overlayList = [];
	...
	NonAbGroupLayer.prototype = new BMap.Overlay();

	NonAbGroupLayer.prototype.initialize = function(map) {
		this._map = map;
		var me = this;
		...
		for(var i = 0; i < allOverlay.length; i++) {
 	    	if("nonABGroup" == allOverlay[i].poiInfo) {
		...
		$.ajax({
			url: $.cxt + "/gridCommon/selectAllNonAbGroupByOrgId", 
	...
	NonAbGroupLayer.prototype.draw = function() {
        this.drawOverlay();
	}

	NonAbGroupLayer.prototype.drawOverlay = function() {
	 	if(this.layer) {
	...
	// 向命名空间注册.
	BMapLib.NonAbGroupLayer = NonAbGroupLayer; 
})()
