...
function initIdxGridList(){
	$.ajax({
		  url:$.cxt + "/firstPage/getMapOptionChannelList", 
		  ...
		  success : function (list){
			 ...
			 for(var i=0,n=list.length;i<n;i++){
				 var grid =list[i];
				 var option= $("<option value='"+grid.CHNL_CODE+"' >"+grid.CHNL_NAME+"</option>");
				if(i==0){
	...
	var chnlCode=$("#idx_gridName").val();
	$.ajax({
		  url:$.cxt + "/firstPage/getMapSignNameByCode", 
		  data:{"chnlCode":chnlCode},
		  type: "POST",
		  async:false,
		  success : function (list){
			 ...
			 for(var i=0;i<list.length;i++){
				 var option= $("<option value='"+list[i].SIGNING_ID+"' >"+"第"+(i+1)+"批</option>");
				 if(i==0){
						option.attr("selected","selected");
					}
				 gridNameSel.append(option);
	...
	$("#idx_gridName").on("change", function(event) {
		var chnlCode=$("#idx_gridName").val();
		$.ajax({
			  url:$.cxt + "/firstPage/getMapSignNameByCode", 
			  data:{"chnlCode":chnlCode},
