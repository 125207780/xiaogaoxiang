...
function gridRankDetailInfoListGrid(orgId){
   ...
   grid.jqGrid({ 
	   url : $.cxt + "/gridRankDetail/getGridRankDetailInfoModify",
	   datatype : "json",
	   mtype : "POST",
 ...
 //查询按钮
	$("#searchList").click(function(){
		reloadBasicUnitJqGrid("search","single");
	});
}
function gridRankInfoListGrid(orgId){
	...
	$("#gridInfoListGrid").GridUnload();
	   grid = $("#gridInfoListGrid"); 
	   grid.jqGrid({ 
		   url : $.cxt + "/gridRank/getGridRankInfoModify",
		   datatype : "json",
		   mtype : "POST",
		   postData : {orgId:orgId1},
		   height : "auto",
		   autowidth : true,
		   colNames: [ '网格','序号','周期类型', '考核周期', '被考核对象', '被考核对象类型','得分','指标得分明细'], 
		   colModel: [ 
			   { name: 'objectId', index: 'objectId',align: 'center',hidden:true}, 
		       { name: 'rowNum', index: 'rowNum',align: 'center'}, 
		       { name: 'evaluateCycleType', index: 'evaluateCycleType',align: 'center',formatter : parseData}, 
		       { name: 'evaluateCycle', index: 'evaluateCycle', align: 'center'}, 
		       { name: 'objectName', index: 'objectName',align: 'center' }, 
		       ...
		       { name: '', index: '',align: 'center',  formatter: function(cellvalue, options, cell){	
		    	   var html = $("<div><div>").append(
		    				$("<span onClick=\"seeDetail('"+cell.objectId+"','"+cell.evaluateCycleType+"','"+cell.evaluateCycle+"','"+cell.objectType+"')\"></span>")
		    				.append("指标得分明细")
		    				.addClass("gridBtns")
		    				.attr("title","指标得分明细")
		    			).html();	
	 ...
	 //查询按钮
		$("#searchList").click(function(){
			reloadBasicUnitJqGrid("search","total");
		});
	}
...
var reloadBasicUnitJqGrid = function(flag,radioflag) {
	if(radioflag=="single"){
	if(flag == undefined) {
		//清空查询条件
		topclear('gridSearch')
	...
	gridOrgId1 = $("#gridName").find("option:selected").val();
	if(gridOrgId1 == "请选择..."){
		gridOrgId1 = $("#girdSaleDept").find("option:selected").val();
		if(gridOrgId1 == '请选择...'){
			gridOrgId1 = $("#girdAreaCompany").find("option:selected").val();
			if(gridOrgId1 == '请选择...'){
				gridOrgId1 = $("#girdCityCompany").find("option:selected").val();
				if(gridOrgId1 == '请选择...'){
					gridOrgId1 = '1'
				}
			}
...
function seeDetail(id,ect,et,ot){
		var msg='指标得分明细';
		topwindow.showWindow({
			   title : msg,
			   data:{},
			   url : $.cxt + "/pages/gis/exam/gridRankDetail.jsp?id="+id+"&&ect="+ect+"&&et="+et+"&&ot="+ot,
		   })
	}
...
function parseLevel(cellValue, options, rowObject) {
	if('gridLevel1'==cellValue){
		return '一类网格';
	}
	if('gridLevel2'==cellValue){
		return '二类网格';
	}
	if('gridLevel3'==cellValue){
		return '三类网格';
	}
	if('gridManager'==cellValue){
		return '网格经理';
	}
	if('cdManager'==cellValue){
		return 'CD政企客户经理';
	}
	if('busiManager'==cellValue){
		return '营业部负责人';
	}
	if('directManager'==cellValue){
		return '直销渠道经理';
	}
	if('societyManager'==cellValue){
		return '社会渠道经理';
	}
	return cellValue;
