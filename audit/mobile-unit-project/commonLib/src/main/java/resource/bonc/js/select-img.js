...
//	Related to the image container
	...
	previewContainerHeight: "193px",
	previewContainerOverflow: "hidden",
	
//	Related to the image
	imagePath: getContextPath(),
	imagePrefix: "",
	imageSuffix: "",
	imageExtension: ""
};
...
jQuery.visualSelect = function(obj, options) {
	...
	var oSelectWidth = oSelect.outerWidth();
	var oSelectHeight = oSelect.outerHeight();
	...
	for (i = 0; i < oSelectOptionsLength; i++) {
		if (jQuery(oSelectOptions[i]).attr("value")) {
			...
			previewImage.attr({
							 src: defaults.imagePath+jQuery(oSelectOptions[i]).attr("value"),
...
jQuery.visualSelect.setOption = function (container, option) {
	...
	jQuery(container + " option[value='" + option + "']").prop("selected", true);
}
