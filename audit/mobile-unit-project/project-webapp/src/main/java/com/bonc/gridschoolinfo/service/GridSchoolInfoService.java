package com.bonc.gridschoolinfo.service;
...
import com.bonc.gridschoolinfo.dao.mapper.GridSchoolInfoMapper;
import com.bonc.school.dao.entity.SchoolUserDetail;

@Service
public class GridSchoolInfoService {

	@Resource
	private GridSchoolInfoMapper mapper;

	public GridSchoolInfoMapper getMapper() {
		return mapper;
	}

	public List<SchoolUserDetail>  selectAll(int statisMonth,String actviType,String usrType,String feeLevel,String gprsLevel,String schoolId,String voiceLevel,String appTypeId,String discFeeId){
		return mapper.selectAll(statisMonth, actviType, usrType, feeLevel, gprsLevel, schoolId,voiceLevel,appTypeId,discFeeId);
	}
 
	public void exportExcel(List<SchoolUserDetail> empList,
			ServletOutputStream outputStream) {
		String[] titles = {"账期(月)","地市编码","地市名称","区县编码","区县名称","学校所在地编码","学校所在地名称","学校编码","学校名称","用户id ","当前主套餐编码","当前主套餐名称","网龄","开户日期","用户类型","性别","年龄","是否4G客户","是否不限量套餐客户 ","是否v网用户","是否宽带用户","是否套餐减免用户","套餐减免优惠编码","套餐减免优惠名称","合约活动类型标识","合约活动类型","合约产品编码","合约活动产品名称","合约活动生效时间","合约活动到期时间","当月ARPU","当月MOU","当月DOU","通话次数","流量分档标识","流量分档","通话时长分档标识","通话时长分档","收入分档标识","收入分档","本月折扣折让金额","终端偏好"};
		try {
			//1、创建工作簿
